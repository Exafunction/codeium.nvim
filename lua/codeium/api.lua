...
function Server.check_status() return status end
...
function Server.load_api_key()
  ...
  if err or type(json) ~= "table" then
    if err == "ENOENT" then
      ...
      vim.defer_fn(function() notify.info(message) end, 100)
    else
...
function Server.authenticate()
  ...
  local function on_submit(value)
    ...
    if config.options.enterprise_mode then
      ...
      if config.options.api.path then endpoint = endpoint .. "/" .. config.options.api.path:gsub("^/", "") end
  ...
  prompt = function() require "codeium.views.auth-menu"(url, on_submit) end
...
function Server:new()
  ...
  function m.is_healthy() return healthy end
  ...
  function m.start()
    ...
    local function on_exit(_, err)
      if not current_cookie then return end
    ...
    if config.options.enable_local_search then table.insert(job_args, "--enable_local_search") end
    ...
    if config.options.enterprise_mode then table.insert(job_args, "--enterprise_mode") end
  ...
  function m.request_completion(document, editor_options, other_documents, callback)
    ...
    this_pending_request = function(is_complete)
      if pending_request[1] == metadata.request_id then pending_request = { 0, noop } end
      ...
      request("CancelRequest", {
        ...
        if err then log.warn("failed to cancel in-flight request", err) end
      end)

      if is_complete then complete(false, nil) end
    end
      ...
      if err then
        ...
        if ok and json then
          if json.state and json.state.state == "CODEIUM_STATE_INACTIVE" then
            if json.state.message then log.debug("completion request failed", json.state.message) end
            return complete(false, nil)
          end
    ...
    return function() this_pending_request(true) end
  end
  ...
  function m.add_workspace()
    ...
    if workspaces[project_root] then return end
